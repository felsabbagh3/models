...
def getGradDict(path, grad_dict):
    ...
    for summary in summary_iterator:
        for v in summary.summary.value:
            if v.tensor:
                if "VarNameRank" in v.tag:  # == 'gradients/FeatureExtractor/MobilenetV1/Conv2d_13_pointwise_2_Conv2d_3_3x3_s2_256/BatchNorm/batchnorm/mul_grad/tuple/control_dependency_1VarNameFeatureExtractor/MobilenetV1/Conv2d_13_pointwise_2_Conv2d_3_3x3_s2_256/BatchNorm/gamma__0':
                    continue
                #     # Get The Tensor Names
                #     tag = v.tag.replace("__0", ":0")
                #     _, variable_tensor_name = tag.split("VarNameRank")
                #     print v.tag
                #     print variable_tensor_name
                #
                #     # Get the Gradient for that particular tensor
                #     rank = np.frombuffer(v.tensor.tensor_content, dtype=np.float32)
                #     print rank
                #
                #     shape = []
                ...
                #         grad_dict[variable_tensor_name]['varname'] = variable_tensor_name
                #         grad_dict[variable_tensor_name]['rank_activation'] = rank
                #     else:
                #         if grad_dict[variable_tensor_name].get('rank_activation', None) is not None:
                #             grad_dict[variable_tensor_name]['rank_activation'] = rank
                    ...
                    if grad_dict.get(variable_tensor_name, None) is None:
                        ...
                        grad_dict[variable_tensor_name]['count'] += 1
                        grad_dict[variable_tensor_name]['sum_grad'] = grad_dict[variable_tensor_name]['sum_grad'] + grad
...
if __name__ == '__main__':
    ...
    with open(act_shapes_file,'rb') as fid:
        act_shapes_list = pickle.loads(fid.read())

    # act_shapes_dict = {}
    # count = 0
    # for varname, vardict in infoDict.iteritems():
    #     act_shapes_dict[varname] = act_shapes_list[count]
    #     count+=1
    #
    # with open(act_shapes_file,'wb') as fid:
    #     fid.write(pickle.dumps(act_shapes_dict))
    ...
    for varname,vardict in infoDict.iteritems():
        ...
        grad_dict[varname]['FLOPS'] = grad_dict[varname]['kH'] * \
                                        grad_dict[varname]['kH'] * \
                                            grad_dict[varname]['in_depth'] * \
                                                grad_dict[varname]['out_depth'] * \
                                                    grad_dict[varname]['rank_activation'][1] * grad_dict[varname]['rank_activation'][2]
